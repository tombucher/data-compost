...
class EPaperVisualizer:
    def __init__(self):
        ...
        try:
            # Initialiser Pygame avec gestion d'erreur
            pygame.init()
            self.screen = pygame.display.set_mode(WINDOW_SIZE)
            pygame.display.set_caption("Data Compost")
    ...
    def _draw_silos_page(self):
        ...
        try:
            ...
            title = f"SILOS {current_silo_page + 1}/{total_silo_pages}"
            ...
            self.max_pages = 2 + total_silo_pages
            page_text = f"< PAGE {self.current_page + 1}/{self.max_pages} >"
            self._draw_text_centered(page_text, self.small_font, TEXT_COLOR, y)
        ...
        try:
            ...
            while running:
                ...
                if self.current_page == 0:
                    ...
                    self._draw_silos_page()  # Cette fonction gère maintenant plusieurs pages de silos
...
def display_compost_info(cn_results_path, silo_info_path=None):
    """Fonction principale pour afficher les informations sur le compost."""
    try:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python compost_info_display.py <cn_results_path> [silo_info_path]")
        sys.exit(1)
